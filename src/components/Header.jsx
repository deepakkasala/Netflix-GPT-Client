...
const Header = () => {
  ...
  return (
    <header className="absolute top-0 left-0 w-full px-4 sm:px-6 md:px-8 py-3 sm:py-4 bg-black/90 sm:bg-gradient-to-b from-black z-50 flex items-center justify-between">
      {/* Netflix Logo */}
      <div className="flex items-center gap-8">
        <img
          onClick={() => (isLoggedIn ? navigate("/browse") : navigate("/"))}
          className="w-24 sm:w-32 md:w-36 lg:w-44 cursor-pointer"
          src={NETFLIX_LOGO}
          alt="Netflix Logo"
        />

        {/* Navbar (only show if user logged in and not on GPT search mode) */}
        {isLoggedIn && !showGptSearch && (
          <nav className="hidden md:flex items-center gap-6">
            <Link
              to="/browse"
              className="text-gray-300 hover:underline hover:cursor-pointer"
